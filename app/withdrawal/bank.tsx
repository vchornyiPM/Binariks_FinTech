...
export default function WithdrawalBankScreen() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      setCalcLoading(true);
      try {
        const result = await bankAccountsService.calculateWithdrawal(selectedCoin.coinSerial, Number(amount));
        setFeeResult(result);
      } catch {
        setFeeResult(null);
      } finally {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setSubmitting(true);
    try {
      await bankAccountsService.createWithdrawalRequest(
      ...
      const msg = err?.response?.data?.message ?? 'Withdrawal request failed. Please try again.';
      showToast(msg, 'error');
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
        ...
        <View style={[styles.header, { borderBottomColor: colors.border }]}>
          ...
          <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>Withdraw via Bank</Text>
          <View style={{ width: 28 }} />
        </View>
          ...
          <View style={styles.stepRow}>
            {[1, 2, 3].map((s) => (
              <View key={s} style={styles.stepItem}>
                <View style={[styles.stepDot, { backgroundColor: (step as number) >= s ? colors.primaryBlue : colors.border }]} />
        ...
        >
          ...
          {step === 1 && (
            <>
              <Text style={[styles.stepTitle, { color: colors.textPrimary }]}>Select wallet to withdraw from</Text>
          ...
          {step === 2 && (
            <>
              <Text style={[styles.stepTitle, { color: colors.textPrimary }]}>Select destination account</Text>
              {loadingAccounts ? (
          ...
          {step === 3 && (
            <>
              ...
              <View style={[styles.summaryCard, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
                <SummaryRow label="From wallet" value={selectedCoin?.name ?? ''} colors={colors} />
                <SummaryRow
                  label="To account"
              ...
              {(calcLoading || feeResult) && (
                <View style={[styles.feeCard, { backgroundColor: colors.cardBg, borderColor: colors.border }]}>
                  {calcLoading ? (
                    ...
                    <>
                      <FeeRow label="Withdrawal amount" value={`${symbol}${feeResult.transactionAmount.toFixed(2)}`} colors={colors} />
                      <FeeRow label="Fee" value={`${symbol}${Math.abs(feeResult.commissionAmountPush).toFixed(2)}`} colors={colors} />
                      <FeeRow label="Total deducted" value={`${symbol}${totalDeducted.toFixed(2)}`} colors={colors} bold />
                    </>
                  ) : null}
                </View>
              )}
              ...
              <Text style={[styles.balanceHint, { color: colors.textSecondary }]}>
                Available: {symbol}{selectedCoin?.availableBalance.toFixed(2) ?? '0.00'} {selectedCoin?.currency.code}
              </Text>

              <TouchableOpacity
                style={[styles.nextBtn, { backgroundColor: (submitting || !amount || calcLoading) ? colors.border : colors.errorRed }]}
          ...
          {step === 'success' && (
            <View style={styles.successContainer}>
              ...
              <Text style={[styles.successSubtitle, { color: colors.textSecondary }]}>
                Your withdrawal of {symbol}{numAmount.toFixed(2)} has been submitted. Funds will be transferred once processed.
              </Text>
              <TouchableOpacity
